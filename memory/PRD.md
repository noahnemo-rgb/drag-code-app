...
## Overview
A React Native / Expo mobile IDE app that lets users code within a GUI. Dark utility aesthetic (Amber on Obsidian), monospaced editor, quick-symbol keyboard strip, drawer-based file explorer, code runner console, and client-side AI (Puter.js on web, OpenRouter BYOK on mobile).
...
- **Code runner** — Executes Python/JS/TS on the FastAPI backend (daily per-device limit). TS uses globally-installed `tsx`. HTML preview rendered on-device via WebView. Output shown in a bottom-sheet Console. 10s timeout.
- **AI Assistant** — Multi-turn chat with streaming responses and local session history. **Web:** Puter.js (user signs in to Puter; usage on their account). **Mobile:** user's OpenRouter API key stored in device keychain (BYOK). Code blocks are syntax-highlighted with Copy, Insert-at-Cursor, and Apply Fix actions.
...
- `POST /run` — `{language, code}` → `{stdout, stderr, exit_code, duration_ms}` (requires `X-Device-Id`, daily limit)
...
## Notes
- AI runs client-side — no LLM key on the backend.
- TypeScript execution falls back to Node (annotations must be JS-compatible unless `tsx` is on PATH).
- Cloud projects/files are scoped by the `X-Device-Id` header.
